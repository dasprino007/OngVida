...
app.post("/cadastrousuario", async (req, res) => {
  ...
  if(nome == null || email == null || endereco == null || numero == null || cep == null || nascimento == null){
    ...
    return res.status(400).json({error : "o email existe no nosso banco de dados"})
  }

  else if(cepTamanho > 11 || cepTamanho < 11){
    return res.status(400).json({error : "tamanho do cep esta errado"})  
  }

  try {
    const newUsuario = await usuario.save();
    res.json({ error: null, msg: "Cadastro ok", UsuarioId: newUsuario._id });
    console.log("cadastro feito do usuario:" + newUsuario._id);
  } catch (error) {}
});
